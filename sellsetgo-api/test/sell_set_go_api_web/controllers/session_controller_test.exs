defmodule SellSetGoApiWeb.SessionControllerTest do
  use SellSetGoApiWeb.ConnCase

  alias SellSetGoApi.Accounts.{Session, Sessions}
  ...
  describe "create a session - generate a new ebay application authorize url" do
    test "renders a authorize url", %{conn: conn} do
      conn = post(conn, Routes.session_path(conn, :create), session: %{})
      assert %{"redirect_uri" => url} = json_response(conn, 201)
    end
  end
